...
setDifficulty newDiff (MkGameState score _) 
= MkGameState score (fromNat newDiff)-}

setDifficulty : Nat -> GameState -> GameState
setDifficulty newDiff state = record { difficulty = fromNat newDiff} state
...
runCommand rnds state GetLine = do str <- getLine
                                   pure (str, rnds, state)
runCommand (value :: rnds) state GetRandom 
    = pure (getRandom value (difficulty state), rnds, state)
  where
...
run (More fuel) rnds state (Quit val) = do pure (Just val, rnds, state)
